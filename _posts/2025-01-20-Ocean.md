...
<img src="/img/butterflyAlgorithm.png" alt="Image" width="400"/>
 <br> *DFT butterfly computations for 8 pixel texture*
...
## Optimizations
<hr>There are multiple optimizations we can do which will mainly be focused on the amount of vertices available we can do a simple CPU frustum culling which will work for most type of games where only a small part of the ocean is visible at a time. This works by having an AABB bound for each tile so the bottom left position of the tile and the top right position of the tile if they are in the view frustum they will be drawn otherwise not.

<video width="420" height="360" controls>
  <source src="/img/FrustumCulling.mp4" type="video/mp4">
</video>

Beside adding frustum culling the performance would also greatly benefit from adding tessellation this will give more vertices/detail up close and use less vertices further away where everything needs less detail.
...
I hope this article was helpful in giving a good insight on this topic and if there is anything you still have questions on just send a e-mail to 230160@buas.nl.
